...
但是这个文件中会出现有一些舰娘的名称是神秘代码 `{namecode:198}` 的情况，推测是反河蟹的大手发力了，找到 `AzurLaneData\CN\ShareCfg\name_code.json` 文件，这里记录了和谐名称和实际名称的映射。

```json title="name_code.json"
  "198": {
    "code": "赛普拉斯",
    "id": 198,
    "name": "莫里森",
...
实际名称“莫里森”使用占位代号 `{namecode:198}` 和谐成了“赛普拉斯”。
...
明确了层级关系之后，我们还需要从 RectTransform 中得到图层的偏移量，缩放等属性。以 `jinluhao` 的 GameObject 绑定的 RectTransform 为例，
...
:::important
- m_LocalPosition：本物体相对于父物体位置的偏移信息，也就是物体自身的枢轴点（Pivot）和父物体的枢轴点（Pivot）之间的相对坐标，与锚点（Anchor）无关。
- m_AnchoredPosition：当物体 Anchor 重合是一个点时，也就是 m_AnchorMin=m_AnchorMax，m_AnchoredPosition 表示物体的 Pivot 和这个 Anchor 之间的相对坐标；当物体的 Anchor 不重合，是一个锚框时，需要先根据 Pivot 映射到锚框中，这个点可以视为锚点，m_AnchoredPosition 表示 Pivot 和锚点之间的相对坐标。[几何意义就是 Pivot 在自身区域的比例，映射到 Anchor 上的点，再与 Pivot 的相对位置，就是 m_AnchoredPosition](https://zhuanlan.zhihu.com/p/194317677)
:::

除此之外还需要关注 MonoBehaviour 中的一个属性，`mRawSpriteSize` 表示贴图原本的大小，也就是说在做上述 Recttransform 之前，我们需要保证我们输入的贴图大小要符合这个属性的尺寸，对于在[立绘还原](#25-立绘还原)中得到的立绘，可能尺寸和这个尺寸不相符，这里就要参考[计算瓜游立绘拼图坐标的亿些补充说明](https://www.bilibili.com/opus/442875260673931811?spm_id_from=333.1007.0.0)这篇文章中的做法，将得到的立绘固定在左下角，然后向右向上填充透明像素以符合尺寸要求。

### 2.7. 表情绘制

正当鼠鼠觉得万事休矣的时候，打算美美开始提取所有舰娘的立绘😍，突然天空一声巨响，无头骑士闪亮登场😭，给我干哪来了，这还是哈尔西鲍威尔吗。

<img src="https://raw.githubusercontent.com/Tz-slayer/image-bed/master/markdown/20250908054852-1757310532267.png" style="width:60%; margin: 0 auto;"/>

稍做分析，[这里](#my-anchor-1)可以发现有一层 `face` 应该是管理表情的图层，推断有些立绘依赖这个 `face` 来显示完整的立绘。在 `AzurLaneData\CN\ShareCfg\ship_skin_expression.json` 中可以看到对应各种情况下的小表情代号。

<details>
<summary>点击展开查看代码</summary>

```json title="ship_skin_expression.json" startLineNumber=20407
  "haerxibaoweier": {
    "battle": "1",
    "default": "0",
    "detail": "2",
...
具体的表情图像文件可以在之前下载的资源文件中的 `/paintingface` 中找到，还是使用 AssetStudioGUI 解包一下就能拿到了。

<img src="https://raw.githubusercontent.com/Tz-slayer/image-bed/master/markdown/20250908062310-1757312590319.png" style="margin: 0 auto;"/>
